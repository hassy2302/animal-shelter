...
- **공고 번호 복사** — 카드 및 상세 모달에서 공고 번호 클릭 시 클립보드 복사
...
- **Inflight dedup** — 동일 키에 대해 fetch가 진행 중이면 후속 요청은 결과를 공유 (thundering herd 방지)
- **`_should_cache`** — 결과가 10건 이하이면 캐시 저장 안 함 (429 등 빈 응답의 캐시 오염 방지)
